...
type PreferenceType = "FIRST_AVAILABLE" | "LAST_AVAILABLE" | "PRICE_ASC" | "PRICE_DESC";
...
const parseFilters = ({ id, value }: { id: string; value: string }) => `specificationFilter_${id}:${value}`;

function getOrdinationProp(attribute: "name" | "value") {
  return Object.keys(ORDER_BY_OPTIONS).map((key) => ORDER_BY_OPTIONS[key as keyof typeof ORDER_BY_OPTIONS][attribute]);
}
...
const ListProductsCollectionOrder = (props: PropsWithChildren<Props>) => {
  ...
  const { data, loading, error } = useQuery<QueryProductsTypes.Data, QueryProductsTypes.Variables>(QueryProducts, {
    variables: {
      category,
      ...(collection != null
        ? {
  ...
  const { products } = data ?? {};
  const [reorderedProducts, setReorderedProducts] = useState<typeof products | undefined>(products);
