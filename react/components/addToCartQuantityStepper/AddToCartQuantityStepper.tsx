...
const CSS_HANDLES = ['containerAddCartButton', 'containerQuantity', 'contentAddedToCartTxt', 'contentQuantity']
...
const AddToCartQuantityStepper = () => {
	...
	useEffect(() => {
		if (selectedItem) {
			...
			setAvailableQuantity(Number(selectedItem?.selectedItem?.sellers[0].commertialOffer.AvailableQuantity))
		}
	}, [selectedItem])
	...
	const isAddedItem = useMemo(() => {
		const foundItemCart = orderForm?.orderForm?.items.find(
			(item: any) => item && selectedItem && selectedItem?.selectedItem?.itemId === item.id
		)
		if (foundItemCart) {
			dispatch({ type: 'SET_QUANTITY', args: { quantity: foundItemCart.quantity } })
			return true
		}
		return false
	...
	return (
		<div className={`${handles.containerAddCartButton} pb4 pt0`}>
			{!isAddedItem ? (
				...
				<div className={`${handles.containerQuantity} `} onClick={handleNumericClick}>
					...
					<span className={`${handles.contentAddedToCartTxt} `}>Adicionado ao carrinho</span>
				</div>
			)}
		</div>
	)
